...
export type AlertKind = 'low_stock' | 'repair_overdue' | 'repair_awaiting_pickup';
 ...
 * and repairs awaiting pickup too long. Warnings first, then info.
 */
export const buildAlerts = ({ inventory, repairs, now, readyStaleDays = READY_PICKUP_STALE_DAYS }: AlertsInput): Alert[] => {
  const alerts: Alert[] = [];
