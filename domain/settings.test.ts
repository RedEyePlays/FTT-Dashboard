...
describe('mergeSettings', () => {
  ...
  it('defaults operations to same-day void, no float, no restocking fee, 30-day aging, 4-min auto-lock, 60-day stale layaway', () => {
    expect(mergeSettings().operations).toEqual({
      openingFloatDefault: 0, voidWindowDays: 0, returnRestockingFeePercent: 0, agingInventoryDays: 30, autoLockMinutes: 4, staleLayawayDays: 60,
    });
  });
  ...
  it('merges a partial operations patch over the defaults', () => {
    ...
    expect(o).toEqual({ openingFloatDefault: 0, voidWindowDays: 3, returnRestockingFeePercent: 15, agingInventoryDays: 30, autoLockMinutes: 4, staleLayawayDays: 60 });
  });
});
