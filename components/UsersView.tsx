...
export const UsersView: React.FC<Props> = ({ me, users, invites, onSetRole, onSetDisabled, onSetAllowProfit, onInvite, onDeleteInvite }) => {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl">
      <div>
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2"><Users className="w-6 h-6 text-indigo-500" /> Users & Roles</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage who can access this shop and what they can do.</p>
      </div>

      {/* Invite */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5">
        <h3 className="font-semibold text-slate-800 dark:text-slate-100 mb-3 flex items-center gap-2"><UserPlus className="w-4 h-4 text-indigo-500" /> Invite a user</h3>
        <div className="flex flex-wrap gap-2 items-end">
          ...
          <div>
            ...
            <select value={role} onChange={e => setRole(e.target.value as Role)} className={sel}>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden">
        <div className="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">Members ({users.length})</h3>
        </div>
