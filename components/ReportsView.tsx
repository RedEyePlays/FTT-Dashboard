...
import { Wallet, Receipt, Download, Save, AlertTriangle, CheckCircle2 } from 'lucide-react';
import { SalesTransaction, CashReconciliation } from '../types';
import { expectedCashForDate, reconcileCash, taxRemittance, taxReportCsvRows, TaxGrouping } from '../domain/reports';
import { toCSV, triggerDownload } from '../services/backup';

interface Props {
  salesTransactions: SalesTransaction[];
  cashReconciliations: CashReconciliation[];
  onSaveReconciliation: (r: { date: string; expectedCash: number; countedCash: number; variance: number; note?: string }) => void;
}
...
export const ReportsView: React.FC<Props> = ({ salesTransactions, cashReconciliations, onSaveReconciliation }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {tab === 'cash'
        ? <CashReconTab salesTransactions={salesTransactions} cashReconciliations={cashReconciliations} onSave={onSaveReconciliation} />
...
const CashReconTab: React.FC<{
  salesTransactions: SalesTransaction[];
  cashReconciliations: CashReconciliation[];
  onSave: Props['onSaveReconciliation'];
}> = ({ salesTransactions, cashReconciliations, onSave }) => {
  const [date, setDate] = useState(todayISO());
  const expected = useMemo(() => expectedCashForDate(salesTransactions, date), [salesTransactions, date]);
  const saved = cashReconciliations.find(r => r.date === date);

  const [counted, setCounted] = useState('');
  const [note, setNote] = useState('');
  // Reset the editable fields whenever the selected day (or its saved record) changes.
  ...
  const save = () => {
    onSave({ date, expectedCash: expected, countedCash: countedNum, variance, note: note.trim() || undefined });
  };
      ...
      <div className={`${card} p-5 space-y-4`}>
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="flex-1 min-w-[160px]">
            <label className={label}>Expected cash (from sales)</label>
            <div className="px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-800 dark:text-slate-100">{money(expected)}</div>
          </div>
          <div>
