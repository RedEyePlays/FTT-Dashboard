...
const App: React.FC = () => {
  ...
  // Mirror the owner's label content padding / line spacing / push-down
  // overrides the same way, so the label modals' HTML/PDF paths apply them
  // live without settings as props.
  useEffect(() => {
    cacheLabelSpacing({ paddingMm: settings.labels.paddingMm, lineGapMm: settings.labels.lineSpacingMm, pushDownMm: settings.labels.contentPushDownMm });
  }, [settings.labels.paddingMm, settings.labels.lineSpacingMm, settings.labels.contentPushDownMm]);
  ...
  const handleSaveSettings = async (next: AppSettings) => {
    ...
    try {
      ...
      cacheLabelSpacing({ paddingMm: next.labels.paddingMm, lineGapMm: next.labels.lineSpacingMm, pushDownMm: next.labels.contentPushDownMm });
    } catch { /* ignore */ }
  };
